...
const ReportTable = ({ reports }: { reports: typeof socialServiceReports }) => (
    <Table>
        <TableHeader>
        <TableRow>
...
export default function SocialServiceReportsPage() {
  ...
  const pendingReports = socialServiceReports.filter(r => r.status === 'Pendiente');
  const archivedReports = socialServiceReports.filter(r => r.status !== 'Pendiente');
